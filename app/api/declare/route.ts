...
export async function POST(req: NextRequest) {
  try {
    ...
    if (partnerSelectError) {
      ...
      return NextResponse.json({ error: 'partner_lookup_failed' }, { status: 500 });
    }
    ...
    const { error: declError } = await admin
      .from('declarations')
      .upsert({ user_id: user.id, partner_id: partnerId, intent }, { onConflict: 'user_id,partner_id', ignoreDuplicates: true });
    if (declError) {
    ...
    if ((count ?? 0) >= 2) {
      ...
      if (!linkedError && linked) {
        const alerts = linked.map((row) => ({ user_id: row.user_id, partner_id: partnerId, status: 'new' as const }));
