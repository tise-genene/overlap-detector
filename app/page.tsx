...
export default function Page() {
  ...
  const [alerts, setAlerts] = useState<Array<{ id: string; status: string; created_at: string; partner_hint: string }>>([]);
  ...
  async function sendMagicLink() {
    ...
    try {
      ...
      const result = isEmail
        ? await supabase.auth.signInWithOtp({ email: destination, options: { emailRedirectTo: redirect } })
  ...
  async function markRead() {
    try {
      const token = await getToken();
      await fetch('/api/alerts/read', { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
      loadAlerts();
    } catch (err) {
      console.error(err);
    }
    ...
    <div>
      <div className="nav">
        <div>
          ...
          <div className="muted">Declare your partner; if they show up elsewhere, you get a generic alert.</div>
        </div>
      ...
      {!isAuthed && (
        <section>
          ...
          <div className="muted" style={{ marginTop: 8 }}>{authMessage}</div>
      ...
      {isAuthed && (
        <>
          <section>
            ...
            <input id="nickname" value={nickname} onChange={(e) => setNickname(e.target.value)} placeholder="Anonymous Owl" />
          ...
          <section>
            ...
            <div className="muted" style={{ marginTop: 8 }}>{declareMessage}</div>
            <div className="small" style={{ marginTop: 8 }}>
              We normalize + hash + salt the identifier on the server. Overlaps trigger generic alerts; we never show who else declared.
            </div>
          </section>
              ...
              <>
                {alerts.map((a) => (
                  <div className="alert" key={a.id}>
                    ...
                    <div className="muted">Partner hash hint: {a.partner_hint || partnerHint('unknown')}</div>
                    <div className="muted">{new Date(a.created_at).toLocaleString()}</div>
                    <div className={`badge ${a.status === 'new' ? 'badge-new' : 'badge-read'}`} style={{ marginTop: 6 }}>
